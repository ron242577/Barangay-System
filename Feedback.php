<?php
/* ============================
   FEEDBACK.PHP (UPDATED NOTIFICATIONS LIKE ADMIN DASHBOARD)
   - Adds FollowUp notifications (hidden if sender email is active)
   - Adds Notification Details Modal (same as Admin Dashboard)
   - Keeps your existing design and feedback features
   ============================ */
...
/* ==========================
   NOTIFICATIONS (COMBINED) + FOLLOWUP (LIKE ADMIN DASHBOARD)
   ========================== */
...
$recentNotifications = array_slice($recentNotifications, 0, 10); // Limit 10
...
if (isset($_POST['action']) && isset($_POST['feedback_id'])) {
    ...
    $action = $_POST['action'];
    $status = ($action === 'approve') ? 'Reviewed' : 'Declined';
    ...
    if ($stmt->execute()) {
        $_SESSION['message'] = "Feedback status updated to $status.";
    } else {
        error_log("Database update failed: " . $stmt->error);
        $_SESSION['message'] = "Error updating feedback status. Please try again.";
    }
    $stmt->close();
...
if (isset($_GET['export']) && $_GET['export'] === 'pdf' && $_SESSION['role'] === 'Admin') {
    ...
    $sql = "SELECT f.id, f.feedback_text, f.status, f.created_at,
    ...
    if ($day_filter === '7') {
        $sql .= " AND DATE(f.created_at) >= DATE_SUB(CURDATE(), INTERVAL 7 DAY)";
    } elseif ($day_filter === '30') {
        $sql .= " AND DATE(f.created_at) >= DATE_SUB(CURDATE(), INTERVAL 30 DAY)";
    } elseif ($day_filter === '365') {
        $sql .= " AND DATE(f.created_at) >= DATE_SUB(CURDATE(), INTERVAL 1 YEAR)";
    ...
    if ($search) {
        $sql .= " AND (a.fullname LIKE ? OR f.feedback_text LIKE ?)";
        $search_term = "%$search%";
        $params[] = $search_term;
        $params[] = $search_term;
        $types .= 'ss';
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        if ($row['status'] === 'Reviewed') $totalReviewed++;
    ...
    $qs = $_GET;
    unset($qs['export']);
    $returnUrl = 'Feedback.php' . (!empty($qs) ? ('?' . http_build_query($qs)) : '');
    ?>
    <!DOCTYPE html>
    <html lang="en">
    <head>
...
if (isset($_GET['export']) && $_GET['export'] === 'csv') {
    ...
    $sql = "SELECT f.id, f.feedback_text, f.status, f.created_at, a.fullname AS resident_name, a.id AS resident_id 
            FROM feedbacks f 
            JOIN accounts a ON f.user_id = a.id 
            WHERE 1=1";
    ...
    $stmt->execute();
    $result = $stmt->get_result();
    $export_feedbacks = $result->fetch_all(MYSQLI_ASSOC);
    $stmt->close();

    header('Content-Type: text/csv');
    header('Content-Disposition: attachment; filename="feedbacks_report.csv"');
    $output = fopen('php://output', 'w');
    fputcsv($output, ['Request ID', 'Resident ID', 'Resident Name', 'Status', 'Feedback Text', 'Created At']);
    foreach ($export_feedbacks as $feedback) {
        fputcsv($output, [
            'FB-' . str_pad($feedback['id'], 3, '0', STR_PAD_LEFT),
            'RES-' . str_pad($feedback['resident_id'], 3, '0', STR_PAD_LEFT),
...
/* Textarea styling for feedback details */
.report-textarea {
  width: 100%;
  height: 120px;
  padding: 8px;
...
<div class="flex1" style="align-items:center; gap: 10px; margin-bottom:10px; flex-wrap:wrap;">
   <div style="display:flex; align-items:flex-start; margin-top: 8px;">
    ...
    <img class="logoadmin" src="images/feedback-icon.png" alt="Logo">
</div>
  <h3 style="margin-top: 8px; margin-left:5px;  color: #443d3d;">Feedback Table</h3>
</div>
  ...
  <div class="flex3">
      <form class="resform"method="GET" action="Feedback.php" id="filterForm">
        ...
        <div>
          <label>Day:</label>
          <select class="custom9" id="feedbackDayFilter" name="day" onchange="filterByFeedbackDay(this.value)">
          ...
          <input type="date" id="feedbackFrom" name="from" value="<?= htmlspecialchars($date_from) ?>" style="display:none;" onchange="applyFeedbackCustomDateFilter()">

          <label id="fbtoo" style="display:none;">To:</label>
          <input type="date" id="feedbackTo" name="to" value="<?= htmlspecialchars($date_to) ?>" style="display:none;" onchange="applyFeedbackCustomDateFilter()">
     </form>
     <form method="GET" action="Feedback.php" id="filterForm">
        <input placeholder="Search" type="search" name="search" value="<?= htmlspecialchars($search) ?>" onchange="document.getElementById('filterForm').submit()" style="margin-left:auto; width:220px;">
      </form>
    </div>
</div>
...
<div id="feedbackModal" style="display:none; position:fixed; top:0; left:0; width:100%; height:100%; background:rgba(0,0,0,0.5); z-index:1000;">
  <div style="position:absolute; top:50%; left:50%; transform:translate(-50%,-50%); background:white; padding:20px; border-radius:5px; width:92%; max-width:600px;">
    ...
    <h2 style="margin-top:0px; margin-bottom:10px;">Feedback Details</h2>
     <hr style="width:99%;">
...
function filterByFeedbackDay(day) {
...
function applyFeedbackCustomDateFilter() {
...
function viewFeedback(feedbackData) {
  try {
    document.getElementById('feedbackDetails').innerHTML = `
      <p><strong>Resident:</strong> ${feedbackData.resident_name || 'N/A'}</p>
      <p><strong>Feedback:</strong></p>
      <textarea class="report-textarea" disabled>${feedbackData.feedback_text || ''}</textarea>
      <p style="margin-top:12px;"><strong>Status:</strong> ${feedbackData.status || 'N/A'}</p>
    `;
...
document.addEventListener('DOMContentLoaded', function() {
  const daySelect = document.getElementById('feedbackDayFilter');
  const fromInput = document.getElementById('feedbackFrom');
  const toInput = document.getElementById('feedbackTo');
  const frLabel = document.getElementById('fbfr');
  const tooLabel = document.getElementById('fbtoo');
